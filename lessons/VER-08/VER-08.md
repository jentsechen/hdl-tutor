# VER-08 | Hierarchical & Reusable RTL Design
...
See
[examples/VER-08/00-function.v](../../examples/VER-08/00-function.v)
...
results from the same two operands.

Invocation: `logic_operations(a, b, and_out, or_out, xor_out);`

Each capability a function doesn't have gets its own runnable example, so
you can run them one at a time instead of all at once:

- [multiple outputs](../../examples/VER-08/10-task-multiple-outputs.v) — the `logic_operations` task above
- [zero inputs](../../examples/VER-08/11-task-zero-input.v) — a task with no arguments at all
- [`#delay`](../../examples/VER-08/12-task-delay.v) — a task that asserts and releases a reset after a fixed delay
- [event control](../../examples/VER-08/13-task-event-control.v) — a task that waits on `@(posedge clk)` in a loop
...
See
[examples/VER-08/20-parameter.v](../../examples/VER-08/20-parameter.v)
...
See
[examples/VER-08/30-conditional-compilation.v](../../examples/VER-08/30-conditional-compilation.v).
...
See
[examples/VER-08/50-force-release.v](../../examples/VER-08/50-force-release.v).
